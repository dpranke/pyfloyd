...
class PythonGenerator(Generator):
    ...
    def generate(self) -> str:
        ...
        if self._options.main:
            vl.append(self._gen_main_header(
                self._options.version, self._options.args
            ))
        else:
            vl.append(self._gen_default_header(
                self._options.version, self._options.args
            ))

        if self._grammar.exception_needed:
            vl.append(self._gen_parsing_runtime_exception_class())

        if self._grammar.operators:
            vl.append(self._gen_operator_state_class())

        vl.append(self._gen_result_class())
        vl.append('\n\n')
        vl.append(self._gen_parse_function())
        vl.append('\n\n')
        vl.append(self._gen_parser_class())

        if self._options.main:
            vl.append(self._gen_main_footer())
        else:
            vl.append(self._gen_default_footer())
        return self._fmt(vl)
    ...
    def _gen_main_header(self, version, args) -> FormatObj:
        ...
        return self._defmtf("""\
            ...
            """, imports=imports, args=args, version=version)
    ...
    def _gen_default_header(self, version, args) -> FormatObj:
        ...
        return self._defmtf("""\
            ...
            # pylint: disable=too-many-lines
            """, imports=imports, version=version, args=args)
    ...
    def _gen_parser_class(self) -> FormatObj:
        ...
        methods += '\n'
        methods.extend(self._gen_methods())
        vl.append(Indent(methods))
        return vl
    ...
    def _gen_methods(self) -> FormatObj:
        ...
        vobj.append(self._gen_rule_methods())

        vobj.append(self._gen_needed_methods())

        for name in self._grammar.needed_builtin_functions:
            vobj.append(self._defmt(self._builtin_methods[f'fn_{name}']))
            vobj.append('\n')
    ...
    def _gen_rule_methods(self) -> VList:
        ...
        for rule, node in self._grammar.rules.items():
            obj.append(self._gen_method_text(rule, node))
        obj.append('\n')

        if self._grammar.needed_builtin_rules:
            for name in sorted(self._grammar.needed_builtin_rules):
                obj.append(self._defmt(self._builtin_methods[f'r_{name}']))
                obj.append('\n')
        return obj
